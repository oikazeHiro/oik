  ...
  <el-dialog v-model="dialogFormVisible" :title="operate === 0?'添加':'设置'">
    ...
    >
      ...
      <el-form-item label="键" prop="keyy">
        <el-input v-model="dictDto.keyy" autocomplete="off"/>
      </el-form-item>
      <el-form-item label="类别" prop="valuee">
...
const validateKeyy = (rule: any, value: any, callback: any) => {
  if (value === '' || !value) {
    callback(new Error('不能为空'))
  } else if (value.length < 2) {
    callback(new Error('字数不能小于2'))
...
const rules = reactive({
  ...
  keyy: [{validator: validateKeyy, trigger: 'blur'}],
})
